...
/* @pjs preload="tswhatis.png", "FPO_map1_1000x385.png", "map_transparent_wcolor.png", "tsmmlogo.png", "BW_regions_0.png", "BW_regions_1.png",
"BW_regions_2.png", "BW_regions_3.png", "BW_regions_4.png" ; */
...
PImage [] regImgs = new PImage [numRegions];
...
color[] colors = {
  color (12, 72, 58), color (56, 123, 43), color (0, 154, 102), color (165, 188, 57), color (178, 169, 126), color (109, 182, 211)
};
...
void setup() {
  ...
  movementMap   = loadImage("1_map_color.png");
  ...
  for (int i = 0; i < numRegions; i++) {
    regImgs[i] = loadImage ("BW_regions_" + i + ".png");
  }
...
void mousePressed() {
 ...
 // println ("mouseX: " + mouseX + " mouseY: " + mouseY);
  ...
  if (orgLinked !=-1) goToLink(orgLinked); //jsonly
  
  if (mouseX < 700) {
  color thisColor = movementMap.get (mouseX + offset, mouseY - offsetY); 
  //color thisColor = movementMap.pixels [(mouseX + offset) + movementMap.width * (mouseY - offsetY)]; 
  for (int i = 0; i < numRegions; i++) {

    if (thisColor > (colors[i] - 100000) && thisColor <( colors[i] + 100000)) {
      markRegion(i); 
    }
  } 
  }
...
void markRegion (int reg) {
  ...
  currentRegion = regions[cur].name;      
  regions[reg].setIsCurrent(true);
  buttons[reg].setIsCurrent(true);
  regions[reg].totalHeight = 0; 
}
...
scrolling for regions w/ one org
